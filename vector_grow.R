...
graph <- ggplot(legend = TRUE) + 
  ...
  geom_line(data = data.vector_grow_rust           , aes(x = size, y = time, colour = "Rust std::vec")) +
  ...
  scale_colour_manual("",
                      breaks = c("LuaJIT table.insert()", "Java Vector<Long>", "Java ArrayList<Long>",
                                 "Java Trove TLongArrayList", "LuaJIT index assign", "Rust std::vec", "C++ std::vector", "C libdynamic vector"),
